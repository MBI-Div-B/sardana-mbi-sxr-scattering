...
@imacro([["checkTape", Type.Boolean, Optional, "check tape"],
...
def acqconf(self, checkTape, checkTarget, checkCamTemp, startTape, stopTape, startTarget, stopTarget, autoModeLaser, autoShutterPump):
    ...
    if checkTape is None:
        checkTape = self.input("Check tape before acquisition:", 
                             data_type=Type.Boolean,
                             title="check tape", default_value=acqConf['checkTape'])

    if checkTarget is None:
        checkTarget = self.input("Check target before acquisition:", 
                             data_type=Type.Boolean,
                             title="check target", default_value=acqConf['checkTarget'])


    if checkCamTemp is None:
        checkCamTemp = self.input("Check cam temp before acquisition:", 
                             data_type=Type.Boolean,
                             title="check cam temp", default_value=acqConf['checkCamTemp'])
    ...
    if autoModeLaser is None:
        autoModeLaser = self.input("Change laser mode automatically before/after scans:", 
                             data_type=Type.Boolean,
                             title="auto mode laser", default_value=acqConf['autoModeLaser'])


    if autoShutterPump is None:
        autoShutterPump = self.input("Open/close pump laser before/after movements:", 
                             data_type=Type.Boolean,
